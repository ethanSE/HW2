...
handlers._tokens.delete = async (data) => {
    ...
    if (id) {
        try {
            // check if user exists
            await _data.read('tokens', id).catch(() => { throw { statusCode: 400, payload: { 'Error': 'Could not find the specified token' } } })
            //delete token
            await _data.delete('tokens', id).catch(() => { throw { statusCode: 500, payload: { 'Error': `could not delete the specified token` } } })
            return { statusCode: 200 }
        } catch (e) {
            console.log(e)
            return (e)
        ...
        return { statusCode: 400, payload: { 'Error': `missing required field: id` } }
    }
};
