    ...
    if (email) {
        try {
            ...
            if (validated) {
                ...
                return { statusCode: 403, payload: { "Error": "Missing required token in header, or token is invalid for user." } }
            }
        } catch (e) {
            console.log(e)
        ...
        try {
            ...
            if (validated) {
                ...
                let userData = await _data.read('users', email)
                    .catch(() => { throw { statusCode: 400, payload: { 'Error': 'Could not find the specified user' } } })

                //delete all orders associated with the user
                await Promise.all(userData.orders.map((orderId) => _data.delete('orders', orderId)))
                    .catch(() => { throw { statusCode: 500, payload: { "Error": "Encountered an error while attempting to delete order from user" } } })

                //delete user
                await _data.delete('users', email)
                    .catch(() => { throw { statusCode: 500, payload: { 'Error': `could not delete the specified user` } } })
