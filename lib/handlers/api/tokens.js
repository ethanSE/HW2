...
tokens.put = async (data) => {
    let tokenId = typeof (data.payload.tokenId) == 'string' && data.payload.tokenId.trim().length === 20 && data.payload.tokenId.trim();
    let extend = typeof (data.payload.extend) == 'boolean' && data.payload.extend == true ? true : false;

    if (tokenId && extend) {
        //get the token data
        try {
            let tokenData = await _data.read('tokens', tokenId).catch(() => { throw { statusCode: 400, payload: { "Error": "The specified token does not exist" } } });
            if (tokenData.expires > Date.now()) {
                //set expiration date to one hour from now
                tokenData.expires = Date.now() + 60 * 60 * 1000;
                await _data.update('tokens', tokenId, tokenData).catch(() => { throw { statusCode: 500, payload: { "Error": "Could not update the token's expiration date" } } });
