...
server.init = () => {
    fs.access('./https', async (error) => {
        if (error) {
            // create directory
            fs.mkdir('./https', { recursive: true }, (err) => {
                if (err) {
                    console.log(err)
                } else {
            ...
            try {
                ...
                let httpsServerOptions = {
                    key: await fs.promises.readFile('./https/key.pem'),
                    cert: await fs.promises.readFile('./https/cert.pem')
                };
                ...
                console.log('error accessing key.pem and cert.pem')
                console.log(`please generate an ssl certificate in the './https' folder`)
                process.exit(0)
            }
        }
    })
