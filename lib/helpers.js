...
helpers.getTemplate = async (templateName, data) => {
    return new Promise(async (resolve, reject) => {
        ...
        if (templateName) {
            ...
            try {
                let template = await fs.promises.readFile(templatesDir + templateName + '.html', 'utf8')
...
helpers.getStaticAsset = (fileName) => {
    return new Promise(async (resolve, reject) => {
        ...
        if (fileName) {
            ...
            try {
                let data = await fs.promises.readFile(publicDir + fileName)
                resolve(data);
            } catch (e) {
                console.log(e)
...
helpers.generateFiles = () => {
    const requiredFolders = ['./.data/users', './.data/tokens', './.data/orders', './https']
    ...
    requiredFolders.forEach(folder => {
        ...
        fs.access(folder, function (error) {
            if (error) {
                ...
                if (folder === './https') {
                    console.log('ssl cert required in ./https')
                    process.exit(0)
                }
            }
        })
